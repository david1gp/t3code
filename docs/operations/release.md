...
- Builds a self-contained CLI archive per platform (`t3-<version>-<platform>-<arch>.tar.gz`, `.zip` on Windows) in the same job as that target's desktop artifact and attaches them to the GitHub Release with a `SHA256SUMS` file, on every channel, for five targets: macOS arm64, Linux x64 and arm64, Windows x64 and arm64. Every archive is built, signed, and smoke-tested on hardware of its own architecture. There is no macOS x64 archive: Node single-executables are unsupported on x64 macOS (the SEA docs list macOS as arm64 only) and the binary segfaults on start; the x64 desktop app is Electron and unaffected.
  - The archive holds the server as a Node single-executable (`scripts/build-cli-archive.ts`), so unpacking it needs neither Node, npm, nor a compiler. It is the only form in which T3 Code manages a runtime: the desktop's SSH environments, the boot service, `t3 update`, and the install scripts all download and verify this archive against `SHA256SUMS`. The npm packages exist for people who run `npx @adaptive-ds/t3code` or `npm install -g @adaptive-ds/t3code` themselves and carry the same archive contents; the installed executable remains `t3`. Nothing in the product installs from npm. The `curl | sh` installers are `scripts/install.sh` and `scripts/install.ps1`; the marketing site copies them into its `public/` at build time (`apps/marketing/scripts/stage-install-scripts.mjs`) and serves them at `t3.codes/install.sh` and `/install.ps1`.
...
- Publishes the CLI to npm with OIDC trusted publishing from the same workflow file, as the same bytes the GitHub Release carries: `scripts/build-npm-platform-packages.ts` unpacks the five CLI archives into `@adaptive-ds/t3code-<platform>-<arch>` packages (each with `os`/`cpu` set so npm installs only the matching one) and generates `@adaptive-ds/t3code`, whose `bin/t3.js` lists them as `optionalDependencies` and execs the installed executable. `npx @adaptive-ds/t3code` therefore needs Node only to run the launcher, never to run the server. `node apps/server/scripts/cli.ts publish` requires all five platform tarballs, publishes them first and the launcher last, after a `--dry-run` pass that checks the tarballs locally. A dry run does not verify npm scope ownership or OIDC authorization. The root workspace manifest intentionally remains private; publish the generated tarballs only.
  - stable releases publish npm dist-tag `latest`
  - nightly releases publish npm dist-tag `nightly`
  - preview releases publish npm dist-tag `preview`, which nothing resolves unless asked for by name
  - one-time setup: the `@adaptive-ds` npm scope (org) must exist, and `@adaptive-ds/t3code` plus each `@adaptive-ds/t3code-<platform>-<arch>` package needs a trusted publisher registered for this workflow file (see below).
...
- Publishes the CLI npm packages (`@adaptive-ds/t3code` and `@adaptive-ds/t3code-<platform>-<arch>`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `@adaptive-ds/t3code@<version>` package available on
...
For a release smoke test, confirm `npm view @adaptive-ds/t3code@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx @adaptive-ds/t3code@<version> service update` once on the
...
1. Confirm the npm org owns the `@adaptive-ds` scope (create the org if
   it does not).
2. For `@adaptive-ds/t3code` and each of the five `@adaptive-ds/t3code-<platform>-<arch>` packages,
   configure a Trusted Publisher in the
   npm package settings (a package that has never been published needs a first publish or a
   placeholder before the setting exists; `--dry-run` does not test registry authorization):
   - Provider: GitHub Actions
   - Organization or user: `david1gp`
   - Repository: `t3code`
   - Workflow filename: `release.yml` (not `.github/workflows/release.yml`)
   - Environment: leave unset (the `publish_cli` job does not use a GitHub environment)
   - Allowed actions: include direct `npm publish` (the workflow does not use staged publishing)
3. Ensure npm account and org policies allow trusted publishing for every package.
4. To build and validate without publishing, first download/extract the five CLI release archives into
   `release-cli`, then run:

   ```sh
   node scripts/build-npm-platform-packages.ts --archives-dir release-cli --version 0.0.42 --output-dir npm-packages
   node apps/server/scripts/cli.ts publish --packages-dir npm-packages --tag latest --provenance --dry-run --verbose
   ```

   The publish target is the generated tarballs, not `npm publish`, `npm publish .`, or any workspace
   directory. Only after the dry run is accepted should an authorized release workflow publish.

5. Stable releases use npm dist-tag `latest`; nightly runs use `nightly`; preview runs use `preview`.
